...
ISR:				; Interrupt????????????
    ; 0x00 saves speed
    ; 0x01 saves direction of counting
    ; 0x0A is the reg that holds num1 passed to DELAY MACRO
    ; 0x0F is counter from 0 to 15 or 15 to 0
    org 0x08			
    CLRF 0x0F
    CLRF LATA
    BTFSC 0x00, 1
...
main:
    ...
    GOTO count_down
    count_up:
	MOVFF 0x01, 0x02; save a copy of 0x01 so we know if there was an interrupt
	MOVLW 0x10
	MOVWF 0x0F
	CLRF LATA
	loop_up:
	    DELAY d'180'	; ?500_000cycles?? ?1MHz???????Delay0.5?
	    INCF LATA
	    DECF 0x0F
	    MOVF 0x01, 0
	    CPFSEQ 0x02
